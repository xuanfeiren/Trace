...
from opto.features.priority_search.utils import set_module_parameters, remap_update_dict
...
class SearchTemplate(Trainer):
    ...
    def train(self,
        ...
        while self.n_epochs < num_epochs or self.n_iters < num_steps:
            ...
            best_candidates, proposals, info_update = self.update(samples, verbose=verbose, **kwargs)

            self.optimizer.update(best_candidates['empirical_mean'].update_dict)  # update self.agent with the proposed parameters
    ...
    def _get_current_parameters_as_update_dict(self, module):
        """Get current parameter values as an update_dict (ParameterNode -> value mapping).
        
        Args:
            module: The module to extract parameters from
            
        Returns:
            dict: Dictionary mapping ParameterNode objects to their current values
        """
        parameters = module.parameters()  # Get list of ParameterNode objects
        update_dict = {param: param.data for param in parameters}
        return update_dict

    def _test_candidate_version(self, candidate, version_name, test_dataset, guide):
        """Test a single version of the best candidate.
        
        Args:
            candidate: ModuleCandidate object to test
            version_name (str): Name of the version (e.g., 'empirical_mean', 'ucb', 'lcb', 'mean_prediction')
            test_dataset: Dataset containing 'inputs' and 'infos'
            guide: Guide to provide feedback
            
        Returns:
            float: Test score for this version
        """
        # Backup current agent parameters (ParameterNode -> value mapping)
        backup_update_dict = self._get_current_parameters_as_update_dict(self.agent)
        
        # Apply candidate's parameters to the agent
        self.optimizer.update(candidate.update_dict)
        
        min_score = self.min_score
        # Get the module from the candidate and test it
        ...
        test_score = self.evaluate(self.agent, guide, test_dataset['inputs'], test_dataset['infos'],
                          ...
                          description=f"Evaluating {version_name} candidate")
        
        # Check if the test_score is within the score range
        if not (self.min_score <= test_score <= self.max_score):
            print(f"Warning: Test score {test_score} for {version_name} is out of the range {self._score_range}.")

        # Restore original agent parameters
        self.optimizer.update(backup_update_dict)
    ...
    def test(self, test_dataset, guide):
        ...
        """
        assert isinstance(self._best_candidates, dict), "self._best_candidates must be a dictionary."
